...
bool keyInstance::reKey(){
    ...
    size_t k64Cnt = (keyLen+63)/64;		/* round up to next multiple of 64 bits */
    ...
    for (size_t i=0;i<subkeyCnt/2;i++){				/* compute round subkeys for PHT */
        DWORD A = f32(i*SK_STEP      ,k32e,keyLen);	/* A uses even key dwords */
        DWORD B = f32(i*SK_STEP+SK_BUMP,k32o,keyLen);	/* B uses odd  key dwords */
        B = ROL(B,8);
